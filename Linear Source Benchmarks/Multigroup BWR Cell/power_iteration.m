...
[F_1_1,F_2_1,H_1,ksi,x_c_t,y_c_t,X_i_c,Y_i_c,s_len,sum_s_len,adj_len,alt_azim_theta,fin_d, mesh_center_abscissa_number,mesh_center_ordinate_number, total_rays,M,c_i_xx(:,:,1), c_i_yy(:,:,1),c_i_xy(:,:,1)]=ray_tracing_for_other_data(X,Y,dx,dy,N_a,sigma_t(:,:,1));
[F_1_2,F_2_2,H_2,ksi,x_c_t,y_c_t,X_i_c,Y_i_c,s_len,sum_s_len,adj_len,alt_azim_theta,fin_d, mesh_center_abscissa_number,mesh_center_ordinate_number, total_rays,M,c_i_xx(:,:,2), c_i_yy(:,:,2),c_i_xy(:,:,2)]=ray_tracing_for_other_data(X,Y,dx,dy,N_a,sigma_t(:,:,2));
size_F=size(F_1_1);
...
size_ray_count_max=size(s_len);
ray_count_maximum=size_ray_count_max(1,5);
...
x_moment_flux=zeros(mesh_center_abscissa_number,mesh_center_ordinate_number,2);
y_moment_flux=zeros(mesh_center_abscissa_number,mesh_center_ordinate_number,2);
k_old=1;
