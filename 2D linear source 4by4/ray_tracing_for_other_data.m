function [F_1,F_2,G_1,G_2,H,tau,ksi,x_c_t,y_c_t,X_i_c,Y_i_c,s_len,sum_s_len,adj_len,alt_azim_theta,fin_d, mesh_center_abscissa_number,mesh_center_ordinate_number, total_rays,M,c_i_xx, c_i_yy,c_i_xy]=ray_tracing_for_other_data(X,Y,dx,dy,N_a,sigma_t)
%M_xx_i and M_yy_i and M_xy_i don't incorporate division by ksi or do they?


[F_1,F_2,G_1,G_2,H,tau,ksi,x_c_t,y_c_t,X_i_c,Y_i_c,s_len,sum_s_len,adj_len,alt_azim_theta,fin_d]=ray_tracing_polar_included(X,Y,dx,dy,N_a,sigma_t);
    ...
    for pol_count=1:polar_discretization_number
            ...
            for p_y=num_y_rays:-1:1
                ...
                while in_dx<=mesh_center_abscissa_number && in_dy<=mesh_center_ordinate_number
                    ...
                    M_xx_i(in_dx,in_dy)=M_xx_i(in_dx,in_dy)+1/(dx*dy)*mu(pol_count,1)*w(pol_count,1)*weight_azimuthal(az_count,1)*fin_d(az_count,1)*((omega_x)^2*t_mki^3/12+t_mki*(x_c_track-X_i_c(in_dx,in_dy))^2);
                    M_yy_i(in_dx,in_dy)=M_yy_i(in_dx,in_dy)+1/(dx*dy)*mu(pol_count,1)*w(pol_count,1)*weight_azimuthal(az_count,1)*fin_d(az_count,1)*((omega_y)^2*t_mki^3/12+t_mki*(y_c_track-Y_i_c(in_dx,in_dy))^2);
