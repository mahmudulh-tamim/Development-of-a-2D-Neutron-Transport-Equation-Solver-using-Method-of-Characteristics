...
it=1
while max(max(max(abs(psi_bound_old-psi_bound))))>tol%max(max(max(max(abs(psi_avg-psi_avg_old)))))>tol
...
mesh(mesh_center_y,mesh_center_x,scaler_flux)
iteration_reflective=it
